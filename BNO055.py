...
class BNO055:
	...
	def begin(self, mode=BNO055.OPERATION_MODE_NDOF):
		# Open I2C bus
	...
	def getVector(self, vectorType):
		...
		xyz = (buf[0] | buf[1] << 8, buf[2] | buf[3] << 8, buf[4] | buf[5] << 8)
		if vectorType == BNO055.VECTOR_MAGNETOMETER:	scalingFactor = 16.0
		else if vectorType == BNO055.VECTOR_GYROSCOPE:	scalingFactor = 900.0
		else if vectorType == BNO055.VECTOR_EULER: 		scalingFactor = 16.0
		else if vectorType == BNO055.VECTOR_GRAVITY:	scalingFactor = 100.0
	...
	def getQuat(self):
		...
		wxyz = (buf[0] | buf[1] << 8, buf[2] | buf[3] << 8, buf[4] | buf[5] << 8, buf[6] | buf[7] << 8)
